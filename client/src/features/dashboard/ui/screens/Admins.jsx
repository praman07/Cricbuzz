...
const Admins = () => {
  ...
  const getRoleBadge = (role) => {
    if (role === "SUPER_ADMIN") {
      return (
        <span className="px-2 py-1 bg-[#fffbeb] text-[#d97706] border border-[#fef3c7] text-[11px] font-bold rounded uppercase tracking-wider">
          Super Admin
        </span>
      );
    }
    return (
      <span className="px-2 py-1 bg-[#f1f5f9] text-slate-600 border border-slate-200 text-[11px] font-bold rounded uppercase tracking-wider">
        Admin
      </span>
    );
  };
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[#e5e7eb] overflow-hidden">
        <div className="p-5 border-b border-[#e5e7eb] flex items-center justify-between bg-[#fafafa]">
          <h3 className="text-[16px] font-bold text-slate-800 flex items-center">
            <Users size={18} className="mr-2 text-[#2ebd4f]" /> Registered
              ...
              {isLoading ? (
                ...
                users.map((user) => (
                  ...
                  >
                    <td className="py-5 px-6">
                      <div className="flex items-center space-x-4">
                        <img
                          src={
                            user.picture ||
                            `https://ui-avatars.com/api/?name=${user.name}&background=1E402F&color=fff`
                          }
                          alt={user.name}
                        ...
                        <div>
                          <p className="font-bold text-[15px] text-slate-900 flex items-center">
                            ...
                            {user._id === employee?._id && (
                              <span className="ml-2 text-[10px] bg-[#e7f9eb] text-[#2ebd4f] px-1.5 py-0.5 rounded font-black uppercase tracking-widest">
                                You
                              </span>
                            )}
                          </p>
                    ...
                    <td className="py-5 px-6">
                      {isSuperAdmin && editingUser === user._id ? (
                        <select
                          value={selectedRole}
                      ...
                      {isSuperAdmin && editingUser === user._id ? (
                        <div className="flex justify-end space-x-2">
                          ...
                          <button
                            ...
                            className="flex items-center text-sm font-bold text-[#15803d] hover:text-[#166534] transition-colors"
                          >
